...
void handleRoot()
{
    ...
    webContent += "<tr>                <td>Błąd Komunikacji Wifi</td>                <td>"
                      ...
                      +
                  String(openThermDev.status.otCommunicationFault ? (openThermDev.status.otCommunicationFault == 1 ? "AKTYWNY" : "NIEAKTYWNY") : "BRAK") + "</td>            </tr>";

    webContent += "<tr>                <td>Błąd Komunikacji Wifi</td>                <td>"

                      +
                  String(openThermDev.status.wifiFault ? (openThermDev.status.wifiFault == 1 ? "AKTYWNY" : "NIEAKTYWNY") : "BRAK") + "</td>            </tr>";

    webContent += "<tr>                <td>Błąd Komunikacji MQTT</td>                <td>"

                      +
                  String(openThermDev.status.mqttFault ? (openThermDev.status.mqttFault == 1 ? "AKTYWNY" : "NIEAKTYWNY") : "BRAK") + "</td>            </tr>";

    webContent += "<tr>                <td>Błąd Pieca</td>                <td>"

                      +
                  String(openThermDev.status.otFault ? (openThermDev.status.otFault == 1 ? "AKTYWNY" : "NIEAKTYWNY") : "BRAK") + "</td>            </tr>";
...
void handleLed()
{
    ...
    if ((openThermDev.status.mqttFault == 1) || (openThermDev.status.wifiFault == 1) || (openThermDev.status.otCommunicationFault == 1))
    {
        digitalRead(openThermDev.ledErrorStatusPin) ? digitalWrite(openThermDev.ledErrorStatusPin, 0x0) : digitalWrite(openThermDev.ledErrorStatusPin, 0x1);
        digitalWrite(openThermDev.ledOKStatusPin, 0x0);
    }
    else
    {
        if ((openThermDev.status.mqttFault == 2) || (openThermDev.status.wifiFault == 2) || (openThermDev.status.otCommunicationFault == 2))
        {
            digitalWrite(openThermDev.ledErrorStatusPin, 0x1);
            digitalWrite(openThermDev.ledOKStatusPin, 0x0);
        }
        else
        {
            digitalWrite(openThermDev.ledErrorStatusPin, 0x0);
            digitalWrite(openThermDev.ledOKStatusPin, 0x1);
        }
    }
    // central heating led is driven low
    if (openThermDev.status.CentralHeating)
    {
        digitalRead(openThermDev.ledCentralHeatingStatusPin) ? digitalWrite(openThermDev.ledCentralHeatingStatusPin, 0x0) : digitalWrite(openThermDev.ledCentralHeatingStatusPin, 0x1);
    }
    else
    {
        if (openThermDev.settings.enableCentralHeating || openThermDev.status.manualCentralHeatinActive)
        {
            digitalWrite(openThermDev.ledCentralHeatingStatusPin, 0x0);
        }
        else
        {
            digitalWrite(openThermDev.ledCentralHeatingStatusPin, 0x1);
        }
    }
    // hot water led
    if (openThermDev.status.HotWater)
    {
        digitalRead(openThermDev.ledHotWaterStatusPin) ? digitalWrite(openThermDev.ledHotWaterStatusPin, 0x0) : digitalWrite(openThermDev.ledHotWaterStatusPin, 0x1);
    }
    else
    {
        if (openThermDev.settings.enableHotWater || openThermDev.status.manualHotWaterActive)
        {
            digitalWrite(openThermDev.ledHotWaterStatusPin, 0x1);
        }
        else
        {
            digitalWrite(openThermDev.ledHotWaterStatusPin, 0x0);
        }
    }
}
